...
```bash
CF_ENV=production yarn cf:deploy
```

`cf:deploy` always runs the build first and verifies `.open-next/worker.js` before calling `npx wrangler deploy --env=""` (or the value of `CF_ENV`).
...
## 5) Compatibility audit (Workers)

- Node-only modules (`fs`, `net`, `tls`, `child_process`) were scanned in `src/` and none are imported directly.
- Middleware uses `fetch` and Web APIs and avoids Node-only SDK usage in Edge runtime.
- JWT helper supports both `atob` (Workers/Web) and `Buffer` fallback.
- Runtime config remains environment-driven (`process.env.*`) and is compatible with Wrangler secrets/vars.
